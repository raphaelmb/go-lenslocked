...
func (service *PasswordResetService) Create(email string) (*PasswordReset, error) {
	...
	pwReset := PasswordReset{UserID: userID, Token: token, TokenHash: service.hash(token), ExpiresAt: time.Now().Add(duration)}
...
func (service *PasswordResetService) Consume(token string) (*User, error) {
	...
	row := service.DB.QueryRow(`SELECT password_resets.id, password_resets.expires_at, users.id, users.email, users.password_hash
